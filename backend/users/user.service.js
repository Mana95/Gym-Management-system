...
async function updateMembership_service(body){
 ...
 await MembershipType.updateOne({
    "membership_type_id": body.membership_type_id
      ...
      return 200;
        ...
        // if(_update){
        //   return {status : 200 , message:"The Membership type is updated successfully!"}
       
        // }else{
        //   return {status : 500 , message:"The Membership name is already available!"}
        // }
...
async function updateInvoice_Controller_service(data) {
  ...
  if(data.invoiceDetails == 'Rejected'){
    ...
    {
      $set:{ paymentDetails: 'Rejected' ,status:'6'}
    }
    )
  }else {
    await Membership.updateOne({
    ...
    {
      $set:{ paymentDetails: 'Sucess',status:'4'}
    }
    )
  }
...
async function saveMembershipReciptDetails_service(data){
  ...
  let membershipData= {
    ...
    invoiceId:data.invoiceId,
    status: '3' // paymentdone by member
  }
...
async function membershipInactive(data) {
    const membership = data.selectedMembership;
    const reasonOb = data.reason;
  ...
  const membershipUpdate = await Membership.updateOne(
    ...
    {
      $set: {status:'7' ,rejectedReason:reasonOb.rejectedReason},
    },
    function (err, responses) {
      if (err) {
        console.log(err);
   ...
   let transporter = nodemailer.createTransport({
    ...
    var mailOptions = {
      to: 'manojdharmarthne@gmail.com',
      from: "your email",
      subject: "Your Membership is inactive",
      text:`HI HI`,
      html:`<!DOCTYPE html>
      <html>
  <head>
              ...
              <tr>
                <td class="wrapper">
                  <table role="presentation" border="0" cellpadding="0" cellspacing="0">
                    <tr>
                      <td>
                        <p>Hey ${membership.firstName},</p>
                        <p>We want to inform you, your current account is inactive reason is for inactive your account <br/>
                        <b class="reason">${reasonOb.rejectedReason}</b></p>
                            ...
                            <tr>
                              <td align="left">
                                <table role="presentation" border="0" cellpadding="0" cellspacing="0">
                                  <tbody>
                                    <tr>
                                      <td> <a href="${config.projectUrl}" target="_blank">Click me</a> </td>
                                    </tr>
                                  </tbody>
                                </table>
                              </td>
                        ...
                        <p>If you want to activate the account please click the above button</p>
