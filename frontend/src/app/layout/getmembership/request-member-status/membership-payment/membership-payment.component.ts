...
export class MembershipPaymentComponent implements OnInit {
  ...
  ngOnInit() {
    this.paymentGroup = this.formBuilder.group({
     ...
     this.paymentRejectGroup = this.formBuilder.group({
      firstName:[''],
      paymentPrice:[''],
      document:['', Validators.required],
      rejectReason:['']
    ...
    this.loadFormData();
    if(this.user.paymentDetails == 'Rejected'){
  ...
  loadInvoiceData(){
    this.paymentRejectGroup.controls['firstName'].setValue(this.user.firstName);
    this.paymentRejectGroup.controls['paymentPrice'].setValue(this.user.amount);
  this.authenticationService.getInvoiceData(this.user.invoiceId)
  .subscribe(
    response=>{
      console.log(response);
      if(response !=undefined){
      this.imageUrl = response.paymentrecipt;
      this.paymentRejectGroup.controls['rejectReason'].setValue(response.rejectedReason);
    }
    }
  )
  }
...
console.log(this.user);
  ...
  onSubmitAgian() {
    ...
    if(this.paymentGroup.valid){
      ...
      //invoice saveing
      this.authenticationService.updateMembershipReciptDetails(invoiceDetails)
      .subscribe(
        response=>{
          console.log(response)
          if(response ==1 ){
  ...
  getDocumentDefinition(cus_data) {
    ...
    return {
      content: [
        ...
        {
          text: 'This is memerbship card valid for ony a on month and please \n make sure to make cle',
          style: 'notesText',
        },
      ],
      styles: {
